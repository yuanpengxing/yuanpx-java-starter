 ...
 *
 */
public final class ImgCmpByAverageHash {

    public static void main(String[] args) {
        try {
            ImgCmpByAverageHash fp1 = new ImgCmpByAverageHash(ImageIO.read(new File("IMG_01.JPG")));
            ImgCmpByAverageHash fp2 = new ImgCmpByAverageHash(ImageIO.read(new File("IMG_02.JPG")));
            System.out.println(fp1.toString(true));
            System.out.printf("sim=%f", fp1.compare(fp2) * 100);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public ImgCmpByAverageHash(byte[] hashValue) {
    ...
    public ImgCmpByAverageHash(String hashValue) {
        this(toBytes(hashValue));
    }

    public ImgCmpByAverageHash(BufferedImage src) {
        this(hashValue(src));
    }
    ...
    /**
     * 从压缩格式指纹创建{@link ImgCmpByAverageHash}对象
     *
     * @param compactValue
     * @return
     */
    public static ImgCmpByAverageHash createFromCompact(byte[] compactValue) {
        return new ImgCmpByAverageHash(uncompact(compactValue));
    }
    ...
    public boolean equals(Object obj) {
        if (obj instanceof ImgCmpByAverageHash) {
            return Arrays.equals(this.binaryzationMatrix,
                    ((ImgCmpByAverageHash) obj).binaryzationMatrix);
        } else
     ...
     * @param compactValue
     * @return
     * @see #compare(ImgCmpByAverageHash)
     */
    ...
    public float compare(String hashValue) {
        return compare(new ImgCmpByAverageHash(hashValue));
    }
    ...
    public float compare(BufferedImage image2) {
        return compare(new ImgCmpByAverageHash(image2));
    }
     ...
     */
    public float compare(ImgCmpByAverageHash src) {
    ...
    private static float compare(byte[] f1, byte[] f2) {
        if (f1.length != f2.length)
            throw new IllegalArgumentException("mismatch ImgCmpByAverageHash length");
        int sameCount = 0;
    ...
    public static float compare(BufferedImage image1, BufferedImage image2) {
        return new ImgCmpByAverageHash(image1).compare(new ImgCmpByAverageHash(image2));
    }
}
